 ...
 * wp11_backend_flash_ops dispatch table using an INDEPENDENT oracle strategy:
 *
 *   Sign:    wp11_backend_flash_ops.sign -> oracle wc_ecc_verify_hash /
 *            wc_RsaSSL_Verify on a fresh key object imported from the same DER.
 *   Verify:  External raw wc_ecc_sign_hash -> wp11_backend_flash_ops.verify.
 *   Decrypt: Oracle wc_RsaPublicEncrypt -> wp11_backend_flash_ops.decrypt.
 *
 * The flash backend is structurally identical to the FSDIR backend: both load
 * keys from encrypted .p11k keystores, decode DER per operation into a
 * temporary wolfCrypt key object, and expose sign/verify/decrypt through
 * wp11_backend_ops_t.  The only difference is the backend type tag
 * (WP11_BACKEND_USB_FLASH vs WP11_BACKEND_FSDIR) and the ops table exported.
 *
 * Compile with -DWOLFP11_CFG_TEST and -DWOLFP11_CFG_USB_FLASH_BACKEND to enable.
 */
...
/* wolfssl/options.h must precede all other wolfssl headers */
...
/* CKM mechanism values -- must match FLASH_CKM_* in wp11_backend_usb_flash.c */
...
static const uint8_t TEST_PIN[]  = { 'f', 'l', 'a', 's', 'h', 'P', 'i', 'n' };
...
static void make_flash_handle(const wp11_key_entry_t *entry,
                               wp11_key_handle_t      *h)
{
    h->backend = WP11_BACKEND_USB_FLASH;
    h->id      = 0;
    h->priv    = (void *)entry;
}
...
static int test_flash_sign_ecdsa(const char *tmpdir)
{
    ...
    f += check(ret == WP11_KEYSTORE_OK,
               "flash_sign_ecdsa: keystore setup succeeds");
    if (ret != WP11_KEYSTORE_OK || ks == NULL) return f;

    entry = wp11_keystore_get(ks, 0);
    f += check(entry != NULL,
               "flash_sign_ecdsa: wp11_keystore_get returns entry");
    if (entry == NULL) { wp11_keystore_free(ks); return f; }

    make_flash_handle(entry, &h);

    ret = wp11_backend_flash_ops.sign(&h, TEST_CKM_ECDSA,
                                      hash, sizeof(hash),
                                      sig, &siglen);
    f += check(ret == 0,    "flash_sign_ecdsa: sign returns 0");
        ...
        if (wc_ecc_init(&oracle_ecc) == 0) {
            if (wc_EccPrivateKeyDecode(der, &idx, &oracle_ecc,
                ...
                f += check(ret == 0 && stat == 1,
                           "flash_sign_ecdsa: oracle wc_ecc_verify_hash confirms signature");
            } else {
                f++;
                printf("FAIL: flash_sign_ecdsa: oracle ecc_key DER import failed\n");
            }
 ...
 * Test 2: ECDSA verify
 *
 * External raw wc_ecc_sign_hash generates a signature; the flash backend
 * verifies it (positive).  Then a tampered signature must be rejected
 * (negative).
 * ---------------------------------------------------------------------- */
static int test_flash_verify_ecdsa(const char *tmpdir)
{
    char                    path[256];
    wp11_keystore_t        *ks    = NULL;
    const wp11_key_entry_t *entry;
    ...
    ret = setup_ec_keystore(path, &ks, der, &der_len);
    f += check(ret == WP11_KEYSTORE_OK,
               "flash_verify_ecdsa: keystore setup succeeds");
    if (ret != WP11_KEYSTORE_OK || ks == NULL) return f;

    entry = wp11_keystore_get(ks, 0);
    f += check(entry != NULL,
               "flash_verify_ecdsa: wp11_keystore_get returns entry");
    if (entry == NULL) { wp11_keystore_free(ks); return f; }
    ...
    if (der_len > 0 && wc_InitRng(&ext_rng) == 0) {
        ...
        if (wc_ecc_init(&ext_ecc) == 0) {
            if (wc_EccPrivateKeyDecode(der, &idx, &ext_ecc,
                ...
                f += check(ret == 0,
                           "flash_verify_ecdsa: external sign succeeds");

                if (ret == 0) {
                    ret = wp11_backend_flash_ops.verify(
                              &h, TEST_CKM_ECDSA,
                              hash, sizeof(hash),
                              sig, (size_t)wlen);
                    f += check(ret == 0,
                               "flash_verify_ecdsa: accepts external signature");
                    ...
                    ret = wp11_backend_flash_ops.verify(
                              ...
                              sig, (size_t)wlen);
                    f += check(ret != 0,
                               "flash_verify_ecdsa: rejects tampered signature");
                }
            } else {
                f++;
                printf("FAIL: flash_verify_ecdsa: external ecc_key DER import failed\n");
            }
            wc_ecc_free(&ext_ecc);
        }
        wc_FreeRng(&ext_rng);
 ...
 *
 * Sign a message with TEST_CKM_ECDSA_SHA256 (flash backend hashes internally).
 ...
 * ---------------------------------------------------------------------- */
static int test_flash_sign_ecdsa_sha256(const char *tmpdir)
{
    char                    path[256];
    wp11_keystore_t        *ks     = NULL;
    const wp11_key_entry_t *entry;
    ...
    ret = setup_ec_keystore(path, &ks, der, &der_len);
    f += check(ret == WP11_KEYSTORE_OK,
               "flash_sign_ecdsa_sha256: keystore setup succeeds");
    if (ret != WP11_KEYSTORE_OK || ks == NULL) return f;

    entry = wp11_keystore_get(ks, 0);
    f += check(entry != NULL,
               "flash_sign_ecdsa_sha256: wp11_keystore_get returns entry");
    if (entry == NULL) { wp11_keystore_free(ks); return f; }

    make_flash_handle(entry, &h);

    ret = wp11_backend_flash_ops.sign(&h, TEST_CKM_ECDSA_SHA256,
                                      msg, sizeof(msg),
                                      sig, &siglen);
    f += check(ret == 0,    "flash_sign_ecdsa_sha256: sign returns 0");
    f += check(siglen > 0 && siglen <= sizeof(sig),
                ...
                if (wc_ecc_init(&oracle_ecc) == 0) {
                    if (wc_EccPrivateKeyDecode(der, &idx, &oracle_ecc,
                        ...
                        ret  = wc_ecc_verify_hash(sig, (word32)siglen,
                                                  ...
                                                  &stat, &oracle_ecc);
                        f += check(ret == 0 && stat == 1,
                                   "flash_sign_ecdsa_sha256: oracle confirms SHA256+ECDSA signature");
                    } else {
                        f++;
                        printf("FAIL: flash_sign_ecdsa_sha256: oracle ecc_key DER import failed\n");
                    }
                    wc_ecc_free(&oracle_ecc);
                }
            }
 ...
 * ---------------------------------------------------------------------- */
static int test_flash_sign_rsa(const char *tmpdir)
{
    char                    path[256];
    wp11_keystore_t        *ks     = NULL;
    const wp11_key_entry_t *entry;
    ...
    f += check(ret == WP11_KEYSTORE_OK,
               "flash_sign_rsa: keystore setup succeeds");
    if (ret != WP11_KEYSTORE_OK || ks == NULL) return f;

    entry = wp11_keystore_get(ks, 0);
    f += check(entry != NULL,
               "flash_sign_rsa: wp11_keystore_get returns entry");
    if (entry == NULL) { wp11_keystore_free(ks); return f; }

    make_flash_handle(entry, &h);

    ret = wp11_backend_flash_ops.sign(&h, TEST_CKM_RSA_PKCS,
                                      hash, sizeof(hash),
                                      sig, &siglen);
        ...
        if (wc_InitRsaKey(&oracle_rsa, NULL) == 0) {
            if (wc_RsaPrivateKeyDecode(der, &idx, &oracle_rsa,
                ...
                f += check(ret == (int)sizeof(hash) &&
                           ...
                           "flash_sign_rsa: oracle wc_RsaSSL_Verify recovers hash");
            } else {
                f++;
                printf("FAIL: flash_sign_rsa: oracle RsaKey DER import failed\n");
            }
 ...
 * from the private DER).  The flash backend decrypts.  The recovered
 * plaintext must equal the original.
 * ---------------------------------------------------------------------- */
static int test_flash_decrypt_rsa(const char *tmpdir)
{
    char                    path[256];
    wp11_keystore_t        *ks    = NULL;
    const wp11_key_entry_t *entry;
    ...
    ret = setup_rsa_keystore(path, &ks, der, &der_len);
    f += check(ret == WP11_KEYSTORE_OK,
               "flash_decrypt_rsa: keystore setup succeeds");
    if (ret != WP11_KEYSTORE_OK || ks == NULL) return f;

    entry = wp11_keystore_get(ks, 0);
    f += check(entry != NULL,
               "flash_decrypt_rsa: wp11_keystore_get returns entry");
    if (entry == NULL) { wp11_keystore_free(ks); return f; }
        ...
        if (wc_InitRsaKey(&enc_rsa, NULL) == 0) {
            if (wc_RsaPrivateKeyDecode(der, &idx, &enc_rsa,
                ...
                ct_len = wc_RsaPublicEncrypt(plaintext, (word32)sizeof(plaintext),
                                             ...
                                             &enc_rsa, &enc_rng);
                f += check(ct_len > 0,
                           "flash_decrypt_rsa: oracle encrypt succeeds");

                if (ct_len > 0) {
                    ret = wp11_backend_flash_ops.decrypt(
                              &h, TEST_CKM_RSA_PKCS,
                              ct, (size_t)ct_len,
                              pt, &ptlen);
                    f += check(ret == 0,
                               "flash_decrypt_rsa: decrypt returns 0");
                    f += check(ptlen == sizeof(plaintext) &&
                               memcmp(pt, plaintext, sizeof(plaintext)) == 0,
                               "flash_decrypt_rsa: recovered plaintext matches original");
                }
            } else {
                f++;
                printf("FAIL: flash_decrypt_rsa: oracle RsaKey DER import failed\n");
            }
...
int wp11_test_backend_usb_flash(void)
{
    char  tmpdir[] = "/tmp/wp11_flash_test_XXXXXX";
    char *tdir;
    int   failures = 0;

    tdir = mkdtemp(tmpdir);
    if (tdir == NULL) {
        printf("FAIL: wp11_test_backend_usb_flash: mkdtemp failed\n");
        return 1;
    }

    failures += test_flash_sign_ecdsa(tdir);
    failures += test_flash_verify_ecdsa(tdir);
    failures += test_flash_sign_ecdsa_sha256(tdir);
    failures += test_flash_sign_rsa(tdir);
    failures += test_flash_decrypt_rsa(tdir);
